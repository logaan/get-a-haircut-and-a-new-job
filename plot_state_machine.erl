...
-export([advance_plot/3]).
-include_lib("eunit/include/eunit.hrl").

% advance_plot(State, SpeakingTo, NPCs) -> {NewState, Message, NewNPCs}
advance_plot(got_job, $D, NPCs) ->
  {NewState, Message} = speak(got_job, $D),
  {NewState, Message, kill_dealer(NPCs)};
advance_plot(got_haircut, $G, NPCs) ->
  {NewState, Message} = speak(got_haircut, $G),
  {NewState, Message, revive_dealer(NPCs)};
advance_plot(State, SpeakingTo, NPCs) ->
...
plot_advancement_test() ->
  ...
  Output = advance_plot(got_laid, $M, dealerfull_world()),
...
revive_dealer_test() ->
  ...
  Output = advance_plot(got_haircut, $G, dealerless_world()),
  ?assertEqual(ExpectedOutput, Output).

kill_dealer_test() ->
  Message = "What the fuck yo- Shi- *cough* *gasp*",
  ExpectedOutput = {killed_dealer, Message, dealerless_world()},
  Output = advance_plot(got_job, $D, dealerfull_world()),
  ?assertEqual(ExpectedOutput, Output).
